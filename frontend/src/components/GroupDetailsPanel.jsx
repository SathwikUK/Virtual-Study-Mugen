...
const GroupDetailsPanel = ({
  ...
  // Handle delete button click with toast notification for unauthorized access.
  const handleDeleteGroupClick = () => {
    if (
      ...
      if (!toast.isActive("unauth-delete")) {
        toast.error("You do not have authority to delete this group.", {
          toastId: "unauth-delete",
          autoClose: 2000,
          icon: false,
        });
      }
  ...
  // Handle invite click: call parent's onInviteClick, then close the modal,
  // and schedule one toast (after window closing) to display "Invitation sent!".
  const handleInviteClick = () => {
    onInviteClick && onInviteClick();
    onClose();
    setTimeout(() => {
      if (!toast.isActive("invite-sent")) {
